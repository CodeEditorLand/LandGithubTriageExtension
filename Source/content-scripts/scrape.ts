...
import { awaitClickable, expectClickable, getCurrentRepo } from "./utils";
	...
	const repoData: Shortcuts = [
		{
			...
			items:
				!area || area === "label"
					? (await scrapeLabels()).map((label) =>
							labelShortcut(label.label, label.color),
						)
					: [],
		},
		{
			...
			items:
				!area || area === "assignee"
					? (await scrapeAssignees()).map((assignee) =>
							assignShortcut(assignee.login, assignee.name),
						)
					: [],
		},
		{
			...
			items:
				!area || area === "milestone"
					? (await scrapeMilestones()).map((milestone) =>
							milestoneShortcut(milestone),
						)
					: [],
		},
	];
...
const scrapeLabels = async () => {
	...
	document
		...
		.forEach((item) => {
			labels.push({
				label: (item.children[2].querySelector(".name") as any)
					.innerText,
