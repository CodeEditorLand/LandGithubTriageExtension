...
import { Message, Shortcut, Shortcuts } from "../common/types";

const fuzzySearchShortcuts = (
	shortcuts: Shortcuts,
	query: string,
): Shortcuts => {
	...
	for (const shortcut of shortcuts) {
		const filteredItems = shortcut.items
			.map((item) => ({
				item,
				score: fuzzyMatchScore(query, item.title),
			}))
	...
	return filtered.sort(
		(a, b) =>
			fuzzyMatchScore(query, b.items[0].title) -
			fuzzyMatchScore(query, a.items[0].title),
	);
};
...
const sendShortcutMessage = async (
	shortcut: Shortcut,
...
const renderInactiveMessage = (container: HTMLElement) => {
	...
	container.appendChild(
		$(
			"p",
			"This extension is only active on `https://github.com/*/issues|pull/*`.",
		),
	);
};
